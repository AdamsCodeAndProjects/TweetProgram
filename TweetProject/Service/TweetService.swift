...
struct TweetService {
    ...
    func uploadTweet(caption: String, completion: @escaping(Bool) -> Void) {
        guard let uid = Auth.auth().currentUser?.uid else { return }  // Will use this to grab the tweet
    ...
    func checkIfUserLikedTweet(_ tweet: Tweet, completion: @escaping(Bool) -> Void) {
        ...
        Firestore.firestore().collection("users")
            ...
            .document(tweetId).getDocument { snapshot, _ in
                guard let snapshot = snapshot else { return }  //  Dont want it to be optional
                completion(snapshot.exists)
            }
    }
    
    func likeTweet(_ tweet: Tweet, completion: @escaping() -> Void) {
        guard let uid = Auth.auth().currentUser?.uid else { return }  //  store the id for who liked the tweet
        guard let tweetId = tweet.id else { return }
        let userLikesRef = Firestore.firestore().collection("users").document(uid).collection("user-likes")
